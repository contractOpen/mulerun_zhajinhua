  ...
  <div class="app">
    ...
    <div v-if="page === 'login'" class="page login-page">
      ...
      <div class="login-card">
        ...
        <div class="login-form">
          ...
          <button class="btn-enter" @click="handleLogin" :disabled="!connected || !effectiveWalletConnected">
    ...
    <div v-if="showWalletModal" class="modal-overlay" @click.self="showWalletModal=false">
      <div class="modal wallet-modal">
        ...
        <p class="modal-desc">{{ t('wallet.connectTonDescription') }}</p>
        <div v-if="walletError" class="wallet-error">{{ walletError }}</div>
        <div class="wallet-options">
          <button class="wallet-opt" @click="handleTonWalletConnect('telegram-wallet')" :disabled="walletConnecting">
            <span class="wallet-opt-icon">💬</span>
            <span class="wallet-opt-name">{{ t('wallet.telegramWallet') }}</span>
            <span class="wallet-opt-chain">{{ t('wallet.recommended') }}</span>
          </button>
          <button class="wallet-opt" @click="handleTonWalletConnect('tonkeeper')" :disabled="walletConnecting">
            <span class="wallet-opt-icon">💎</span>
            <span class="wallet-opt-name">{{ t('wallet.tonkeeper') }}</span>
            <span class="wallet-opt-chain">TON</span>
          </button>
          <button class="wallet-opt secondary" @click="handleTonWalletConnect()" :disabled="walletConnecting">
            <span class="wallet-opt-icon">⋯</span>
            <span class="wallet-opt-name">{{ t('wallet.otherTonWallets') }}</span>
            <span class="wallet-opt-chain">{{ t('wallet.allWallets') }}</span>
          </button>
        </div>
        <div class="wallet-manual">
          <label class="wallet-manual-label">{{ t('wallet.tonAddress') }}</label>
          <input
            v-model.trim="manualTonAddress"
            class="wallet-manual-input"
            placeholder="EQ... / UQ... / 0:..."
            autocapitalize="off"
            autocomplete="off"
            spellcheck="false"
          />
          <button class="wallet-manual-btn" @click="handleManualWalletConnect">
            {{ t('wallet.useAddressWithoutAuth') }}
          </button>
        </div>
...
const { init: initTg, userName, userLanguage, showAlert } = useTelegram()
...
const appMode = typeof __APP_MODE__ !== 'undefined' ? __APP_MODE__ : 'tg'
...
onMounted(() => {
  initTg()
  connect()
  playerName.value = userName.value || (t('common.player') + Math.floor(Math.random() * 1000))
...
watch(connected, (isConnected) => {
  if (!isConnected || authenticated.value) return
...
const effectiveWalletConnected = computed(() => {
  return realWalletConnected.value
})
const effectiveWalletAddress = computed(() => {
  return realWalletAddress.value
})
...
function handleWalletConnect() {
  playSound('click')
  manualTonAddress.value = effectiveWalletAddress.value || ''
  showWalletModal.value = true
}
async function handleTonWalletConnect(preferredWalletAppName = '') {
  try {
    await connectTON(preferredWalletAppName)
    manualTonAddress.value = realWalletAddress.value || manualTonAddress.value
    if (realWalletConnected.value) {
      showWalletModal.value = false
      showNotice(t('wallet.connectedTon'), 'success')
    }
  } catch (_) {}
}
function handleManualWalletConnect() {
  const address = manualTonAddress.value.trim()
  if (!validateAddress(address, 'ton')) {
    const message = t('wallet.invalidTonAddress')
  ...
  showWalletModal.value = false
  showNotice('TON address saved without signature authorization', 'success')
}
async function handleLogin() {
  if (!playerName.value.trim()) playerName.value = t('common.player')+Math.floor(Math.random()*1000)
  playSound('click')

  if (!effectiveWalletConnected.value) {
      const message = t('login.walletRequired')
    showNotice(message, 'info', 4200)
    showAlert(message)
    return
  }
...
.wallet-opt.secondary { background:rgba(255,255,255,0.02); }
...
.wallet-manual { display:flex; flex-direction:column; gap:8px; margin-bottom:12px; }
.wallet-manual-label { font-size:12px; color:var(--text-dim); }
.wallet-manual-input { width:100%; padding:12px 14px; border-radius:10px; border:1px solid rgba(255,255,255,0.12); background:rgba(255,255,255,0.05); color:var(--text); font-size:13px; }
.wallet-manual-input::placeholder { color:rgba(255,255,255,0.35); }
.wallet-manual-btn { width:100%; padding:10px 14px; border-radius:10px; border:1px solid rgba(255,215,0,0.18); background:rgba(255,215,0,0.08); color:#f6e7c8; font-size:13px; cursor:pointer; }
.wallet-manual-btn:hover { border-color:rgba(255,215,0,0.45); background:rgba(255,215,0,0.12); }
